...
def main():
    ...
    if platform == "x":
        provider = os.environ.get("PROMOTE_X_PROVIDER", "direct").strip().lower()
        direct_missing = [
            name for env_key, name in checks["x"] if not os.environ.get(env_key)
        ]
        xquik_missing = [
            name
            for env_key, name in [
                ("PROMOTE_XQUIK_API_KEY", "Xquik API Key"),
                ("PROMOTE_XQUIK_ACCOUNT", "Xquik Account"),
            ]
            if not os.environ.get(env_key)
        ]
        missing = xquik_missing if provider == "xquik" else direct_missing
    else:
    ...
    print("Run auth_status for setup instructions.", file=sys.stderr)
    sys.exit(2)
